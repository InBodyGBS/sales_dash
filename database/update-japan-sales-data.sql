-- ============================================
-- Japan Entity Item Mapping 업데이트 스크립트
-- 우선순위: item_master → item_mapping
-- ============================================
...
-- 이 업데이트는 모든 entity에 적용됨 (item_master는 global)
...
-- item_master에 없는 경우만 item_mapping 사용
UPDATE sales_data sd
SET 
...
-- Step 6: 업데이트 통계
SELECT 
    'Update Statistics' as info,
    COUNT(DISTINCT item_number) as unique_items_updated,
    COUNT(*) as total_records_updated
FROM sales_data
WHERE entity = 'Japan'
  AND item_number IS NOT NULL
  AND (
    fg_classification IS NOT NULL OR
    category IS NOT NULL OR
    model IS NOT NULL OR
    product IS NOT NULL
  );
